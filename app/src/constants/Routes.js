// const BASE_ADDRESS = "http://localhost:3001/#";

export const TDD_URL = "https://tdd.ai";
export const DATASETS_URL = "https://data.tdd.ai";
export const APP_ADDRESS_URL = "https://mukayese.tdd.ai";

export const ABOUT_PATH = "https://tdd.ai/?page_id=83&language=en"
export const CONTRIBUTORS_PATH = "https://tdd.ai/?page_id=236&language=en"

export const TASKS_PATH = "/data/tasks.json";
export const TASKS_DETAILS_PATH = "/data/tasks/";
export const SUBMIT_PATH = "/data/submit.md";
